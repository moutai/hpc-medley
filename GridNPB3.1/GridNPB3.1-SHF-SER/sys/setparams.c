...
void write_mf_info(FILE *fp, char class);
...
enum benchmark_types {SP, BT, LU, MG, FT, MF};
...
void get_info(char *argv[], int *typep, char *classp) 
{
  ...
  *classp = *argv[2];
  ...
  else if (!strcmp(argv[1], "mf") || !strcmp(argv[1], "MF")) *typep = MF;
  else {
...
void check_info(int type, char class) 
{
  ...
  if (class != 'S' && 
    ...
    printf("setparams: Allowed classes are \"S\", \"A\", \"B\" and \"W\"\n");
    exit(1);
  }
...
/* 
 * write_mf_info(): Write MF specific info to config file
 */

void write_mf_info(FILE *fp, char class) 
{
  int cfdsize, mgsize, ftsize, maxsize;

  if( class == 'S' )
   {cfdsize = 5*12*12*12;    mgsize = 32*32*32;    ftsize = 64*64*64;}
  else if( class == 'W' )
   {cfdsize = 5*33*33*33;    mgsize = 128*128*128; ftsize = 128*128*32;}
  else if( class == 'A' )
   {cfdsize = 5*64*64*64;    mgsize = 256*256*256; ftsize = 256*256*128;}
  else if( class == 'B' )
   {cfdsize = 5*102*102*102; mgsize = 256*256*256; ftsize = 512*256*256;}
  else if( class == 'C' )
   {cfdsize = 5*162*162*162; mgsize = 512*512*512; ftsize = 512*512*512;}
  else
  {
    printf("setparams: Internal error: invalid class type %c\n", class);
    exit(1);
  ...
  maxsize = max(cfdsize,max(mgsize,ftsize));

  fprintf( fp, "%sinteger    SIZE\n", FINDENT );
  fprintf( fp, "%sparameter( SIZE=%d)\n", FINDENT, maxsize );
...
void write_ft_info(FILE *fp, char class) 
{
  ...
  fprintf(fp, "%sinteger nx, ny, nz, maxdim, niter_default, ntotal\n", FINDENT);
