...
c---------------------------------------------------------------------
      ...
      subroutine exact_rhs(step, fullname, width, depth, ngbclass)
...
c---------------------------------------------------------------------
      ...
      double precision dtemp(5), xi, eta, zeta, dtpp, fac
      integer          m, i, j, k, ip1, im1, jp1, jm1, km1, kp1, step
      character        name*2, fullname*5, tail*2, ngbclass
      integer          width, depth, intfac
...
c---------------------------------------------------------------------
c     now change the sign of the forcing function, and modify in case
c     of rapid fire
c---------------------------------------------------------------------

      if (tail .ne. 'RF' .or. ngbclass .eq. 'S') then
        intfac = 0
      else
        if (name .eq. 'HC') then
          intfac = step
        elseif (name .eq. 'VP' .or. name .eq. 'MB') then
          intfac = step/width
        endif
      endif
      fac = dsin(dble(intfac))

      do k = 1, grid_points(3)-2
         do j = 1, grid_points(2)-2
            do i = 1, grid_points(1)-2
      	       do m = 1, 5
                  forcing(m,i,j,k) = (-1.d0 + fac) * forcing(m,i,j,k)
               enddo
            enddo
         enddo
      enddo
