...
void zone_max_xysize(double ratio, int gx_size, int gy_size,
		   ...
		   int *max_xysize, int *max_xybcsize,
		   int *max_numzones, int *max_lsize);
...
void check_info(char *benchmark, int type, int nprocs, char class) 
{
  ...
  if (class != 'S' && 
    ...
    printf("setparams: Allowed classes are \"S\", \"W\", \"A\",");
    printf(" \"B\", \"C\" and \"D\"\n");
    exit(1);
  }
...
void write_sp_info(FILE *fp, int nprocs, char class) 
{
  ...
  int max_xysize, max_xybcsize, max_numzones, max_lsize;
  char *dt, *ratio;;
  ...
  fprintf(fp, "%sinteger num_procs\n", FINDENT);
  fprintf(fp, "%sparameter (num_procs=%d)\n", FINDENT, nprocs);
  ...
  fprintf(fp, "%sinteger max_xysize, max_xybcsize\n", FINDENT);
  fprintf(fp, "%sparameter (max_xysize=%d, max_xybcsize=%d)\n", FINDENT, 
      	  max_xysize, max_xybcsize);
 ...
 */
   ...
   jzone  = zone/x_zones;
   izone  = (zone % x_zones);
   izone_west  = ((izone-1+x_zones) % x_zones);
   izone_east  = ((izone+1)	    % x_zones);
   jzone_south = ((jzone-1+y_zones) % y_zones);
   jzone_north = ((jzone+1)	    % y_zones);
   id_west   = izone_west + jzone*x_zones;
   id_east   = izone_east + jzone*x_zones;
   id_south  = izone + jzone_south*x_zones;
   id_north  = izone + jzone_north*x_zones;

   comm_index = 0;
   for (iz = 0; iz < num_zones; iz++) {
      if (proc_zone_id[iz] == iproc) {
     	 if ((iz == id_west) || (iz == id_east))
     	    comm_index = comm_index + y_size[jzone];
     	 else if ((iz == id_south) || (iz == id_north))
		   ...
		   int *max_numzones, int *max_lsize)
{
   ...
   int iz, mz, z2, i, j, cur_size, ip, np;
   double x_r0, y_r0, x_r, y_r, x_smallest, y_smallest, aratio;

   int x_size[x_zones], y_size[y_zones];
   int zone_size[num_zones], z_order[num_zones];
   int proc_zone_size[num_procs], proc_num_zones[num_procs];
   int proc_zone_size2[num_procs], proc_xybc_size[num_procs];
#ifndef BLOAD_NO_COMM
   int proc_zone_id[num_zones], zone, zone_comm, comm_index, tratio;
   double diff_ratio;
#endif
...
/* ... use a simple bin-packing scheme to balance the load among processes */
   ...
   while (iz < num_zones) {
      ...
      mz = z_order[iz];
      if (iz < num_zones-1) {
     	 zone_comm = get_comm_index(mz, np, x_zones, y_zones, 
   	 ...
   	 for (z2 = iz+1; tratio && z2 < num_zones; z2++) {
   	    ...
   	    if (diff_ratio > 0.05) tratio = 0;
     	    ...
     	    if (tratio && proc_zone_id[zone] < 0) {
     	       comm_index = get_comm_index(zone, np, x_zones, y_zones, 
   				           proc_zone_id, x_size, y_size);
