c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>c
c
      subroutine smp_setup(num_zones, nx, ny, nz, tot_threads)
c
c  Set up SMP related work, including
c     - zone-process mapping for load balance
...
c
      include 'smp_stuff.h'
c
      integer zone_count, iz, size
c
c ... initialize SMP parameters
      call smp_init
c
c ... map zones to SMP processes
      call map_zones(num_zones, nx, ny, nz, tot_threads)
c
c ... allocate shared memory for data exchange
      size = 10*((gx_size-2*x_zones)*y_zones+
     &           (gy_size-2*y_zones)*x_zones)*(gz_size-2)
      call smp_getshmemp(8, size, qbc_ptr)
      size = num_procs * 15
      call smp_getshmemp(8, size, sbuff_ptr)
c
c ... fork SMP processes
      myid = smp_forkthread(num_procs, proc_num_threads)
      root = 0
c
...
c
      if (mp .le. 0) goto 80
c
      open(unit=2, file='loadlu-mz.data', status='old', iostat=ios)
      if (ios.eq.0) then
