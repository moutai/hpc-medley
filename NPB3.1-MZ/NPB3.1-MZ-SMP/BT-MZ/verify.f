...
c---------------------------------------------------------------------
        ...
        double precision rho_i(*), us(*), vs(*), ws(*), 
     ...
     $                   xce_sub(5), xcr_sub(5)
        ...
        integer start1(*), start5(*), nx(*), nxmax(*), ny(*), nz(*)
        logical verified
...
c---------------------------------------------------------------------
     ...
     $                     nx(zone), nxmax(zone), ny(zone), nz(zone))
        ...
        do m = 1, 5
          sbuffer(m, myid) = xcr(m)
          sbuffer(m+5, myid) = xce(m)
        end do
        call smp_barrier
        do ip = 0, num_procs-1
      	   if (ip .ne. myid) then
             do m = 1, 5
               xcr(m) = xcr(m) + sbuffer(m, ip)
               xce(m) = xce(m) + sbuffer(m+5, ip)
             end do
	   endif
        end do
